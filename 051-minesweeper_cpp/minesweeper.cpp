...
int countMines(int x, int y) {
    /* write code*/
    int mineCount = 0;
    for (int dy = max(0, y - 1); dy < min(y + 2, height); ++dy) {
        for (int dx = max(0, x - 1); dx < min(x + 2, width); ++dx) {
            if (dy == y && dx == x) 
            {
                continue;
            }
            if (board[dy][dx].is_mine()) {
                mineCount += 1;
            }
        }
    }
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                if (board[y][x].is_unknown()) {
                    return false; // There are still unrevealed non-mined squares
                }
            }
        }
        return true;
